...
async def _queue_to_generator(
    queue: asyncio.Queue, queue_event: asyncio.Event, finish_event: asyncio.Event
):
    while True:
...
async def _multiplex(
    ...
    queue_list: list[asyncio.Queue],
    queue_event_list: list[asyncio.Event],
...
def multiplexor(
    func_d: dict[str, Callable[[AsyncGenerator], AsyncGenerator | None]], in_stream: AsyncGenerator
    ...
    multi_task = asyncio.create_task(
        _multiplex(in_stream, queue_list, queue_event_list, stream_end_event)
    )
