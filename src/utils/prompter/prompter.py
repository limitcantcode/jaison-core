...
class Prompter(metaclass=Singleton):
    ...
    async def configure(self, config_d: dict[str, Any]):
        if "instruction_prompt_filename" in config_d:
            self.instruction_prompt_filename = str(config_d["instruction_prompt_filename"])
        if "character_prompt_filename" in config_d:
            self.character_prompt_filename = str(config_d["character_prompt_filename"])
        if "scene_prompt_filename" in config_d:
            self.scene_prompt_filename = str(config_d["scene_prompt_filename"])
        if "character_name" in config_d:
            self.character_name = str(config_d["character_name"])
        if "name_translations" in config_d:
            self.name_translations = dict(config_d["name_translations"])
        if "history_length" in config_d:
            self.history_length = int(config_d["history_length"])

        assert (
            self.instruction_prompt_filename is not None
            and len(self.instruction_prompt_filename) > 0
            and os.path.isfile(
                portable_path(
                    os.path.join(
                        Config().PROMPT_DIR,
        ...
        assert (
            self.character_prompt_filename is not None
            and len(self.character_prompt_filename) > 0
            and os.path.isfile(
                portable_path(
                    os.path.join(
                        Config().PROMPT_DIR,
        ...
        assert (
            self.scene_prompt_filename is not None
            and len(self.scene_prompt_filename) > 0
            and os.path.isfile(
                portable_path(
                    os.path.join(
                        Config().PROMPT_DIR,
    ...
    # Custom context
    def register_custom_context(
        self, context_id: str, context_name: str, context_description: str = None
    ):
    ...
    async def add_chat_stream(
        self, name: str, in_stream: AsyncGenerator, time: datetime.datetime = None
    ):
        if time is None:
            time = get_current_time(include_ms=False, as_str=False)
    ...
    def get_context_descriptions(self):
        ...
        for context_id in self.context_metadata:
            result += f"{self.context_metadata[context_id].name}: {self.context_metadata[context_id].description}\n"
    ...
    def get_sys_prompt(self):
        return f"{self.get_instructions_prompt()}\n{self.response_template}\n{self.get_context_descriptions()}\n### Character ###\n{self.get_character_prompt()}\n### Scene ###\n{self.get_scene_prompt()}"
