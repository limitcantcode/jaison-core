...
class UnknownField(Exception):
    def __init__(self, field: str):
        super().__init__(f"Config field {field} does not exist")
...
class Config(metaclass=Singleton):
    ...
    prompter: dict = {}
    history_filepath: str = portable_path(
        os.path.join(os.getcwd(), "output", "history.txt")
    )  # debug
    ...
    def __init__(self):
        ...
        if args.config is not None:
            self.load_from_name(args.config)
    ...
    def load_from_dict(self, **conf_d):
        ...
        for field in conf_d:
            ...
            uncommitted[field] = (
                config_typings[field](conf_d[field]) if conf_d[field] is not None else None
            )  # attempt cast to correct typing
