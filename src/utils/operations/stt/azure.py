...
class AzureSTT(STTOperation):
    ...
    async def start(self) -> None:
        ...
        self.speech_config = speechsdk.SpeechConfig(
            region=os.environ.get("AZURE_REGION"), subscription=os.getenv("AZURE_API_KEY")
        )

    async def configure(self, config_d):
        """Configure and validate operation-specific configuration"""
        if "language" in config_d:
            self.model_id = str(config_d["language"])
    ...
    async def _generate(
        ...
        **kwargs,
        ...
        transcriber = speechsdk.transcription.ConversationTranscriber(
            speech_config=self.speech_config, audio_config=audio_config, language=self.language
        )
