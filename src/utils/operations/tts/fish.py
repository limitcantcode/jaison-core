...
class FishTTS(TTSOperation):
    ...
    async def configure(self, config_d):
        ...
        if "backend" in config_d:
            self.backend = str(config_d["backend"])
        if "normalize" in config_d:
            self.normalize = bool(config_d["normalize"])
        if "latency" in config_d:
            self.latency = str(config_d["latency"])
    ...
    async def _generate(self, content: str = None, **kwargs):
        ...
        b = b""
        async for chunk in self.session.tts(tts_request, self._stream(), backend=self.backend):
            b += chunk
