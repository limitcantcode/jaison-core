...
class PyttsTTS(TTSOperation):
    ...
    async def start(self):
        ...
        logging.info(f"Operation {self.op_id}: Available voices are: {[x.id for x in voices]}")
    ...
    async def configure(self, config_d):
        ...
        if "gender" in config_d:
            self.gender = str(config_d["gender"])
        if "working_file" in config_d:
            self.working_file = str(config_d["working_file"])
    ...
    async def get_configuration(self):
        ...
        return {"voice": self.voice, "gender": self.gender, "working_file": self.working_file}
