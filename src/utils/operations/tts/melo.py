...
class MeloTTS(TTSOperation):
    ...
    async def configure(self, config_d):
        ...
        if config_d.get("config_filepath", None):
            self.config_filepath = str(config_d["config_filepath"])
        if config_d.get("model_filepath", None):
            self.model_filepath = str(config_d["model_filepath"])
        if "speaker_id" in config_d:
            self.speaker_id = str(config_d["speaker_id"])
        if "device" in config_d:
            self.device = str(config_d["device"])
        if "language" in config_d:
            self.language = str(config_d["language"])

        if "sdp_ratio" in config_d:
            self.sdp_ratio = float(config_d["sdp_ratio"])
        if "noise_scale" in config_d:
            self.noise_scale = float(config_d["noise_scale"])
        if "noise_scale_w" in config_d:
            self.noise_scale_w = float(config_d["noise_scale_w"])
