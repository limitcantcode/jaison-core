...
class KoboldT2T(T2TOperation):
    ...
    async def configure(self, config_d):
        ...
        if "max_context_length" in config_d:
            self.max_context_length = config_d["max_context_length"]
        if "max_length" in config_d:
            self.max_length = config_d["max_length"]
        if "rep_pen" in config_d:
            self.rep_pen = config_d["rep_pen"]
        if "rep_pen_range" in config_d:
            self.rep_pen_range = config_d["rep_pen_range"]
        if "rep_pen_slope" in config_d:
            self.rep_pen_slope = config_d["rep_pen_slope"]
        if "temperature " in config_d:
            self.temperature = config_d["temperature "]
        if "tfs" in config_d:
            self.tfs = config_d["tfs"]
        if "top_a" in config_d:
            self.top_a = config_d["top_a"]
        if "top_k" in config_d:
            self.top_k = config_d["top_k"]
        if "top_p" in config_d:
            self.top_p = config_d["top_p"]
        if "typical" in config_d:
            self.typical = config_d["typical"]
