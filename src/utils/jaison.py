...
class JAIson(metaclass=Singleton):
    ...
    async def create_job(self, job_type: Enum, job_id: str, **kwargs):
        ...
        if job_type_enum == JobType.RESPONSE:
            coro = self.response_pipeline(job_id, job_type_enum, **kwargs)
        elif job_type_enum == JobType.CONTEXT_REQUEST_ADD:
            coro = self.append_request_context(job_id, job_type_enum, **kwargs)
        elif job_type_enum == JobType.CONTEXT_CONVERSATION_ADD_TEXT:
            coro = self.append_conversation_context_text(job_id, job_type_enum, **kwargs)
        elif job_type_enum == JobType.CONTEXT_CONVERSATION_ADD_AUDIO:
            coro = self.append_conversation_context_audio(job_id, job_type_enum, **kwargs)
        elif job_type_enum == JobType.CONTEXT_CLEAR:
            coro = self.clear_context(job_id, job_type_enum, **kwargs)
        elif job_type_enum == JobType.CONTEXT_CONFIGURE:
            coro = self.configure_context(job_id, job_type_enum, **kwargs)
        elif job_type_enum == JobType.CONTEXT_CUSTOM_REGISTER:
            coro = self.register_custom_context(job_id, job_type_enum, **kwargs)
        elif job_type_enum == JobType.CONTEXT_CUSTOM_REMOVE:
            coro = self.remove_custom_context(job_id, job_type_enum, **kwargs)
        elif job_type_enum == JobType.CONTEXT_CUSTOM_ADD:
            coro = self.add_custom_context(job_id, job_type_enum, **kwargs)
        elif job_type_enum == JobType.OPERATION_LOAD:
            coro = self.load_operations(job_id, job_type_enum, **kwargs)
        elif job_type_enum == JobType.OPERATION_CONFIG_RELOAD:
            coro = self.load_operations_from_config(job_id, job_type_enum, **kwargs)
        elif job_type_enum == JobType.OPERATION_UNLOAD:
            coro = self.unload_operations(job_id, job_type_enum, **kwargs)
        elif job_type_enum == JobType.OPERATION_CONFIGURE:
            coro = self.configure_operations(job_id, job_type_enum, **kwargs)
        elif job_type_enum == JobType.OPERATION_USE:
            coro = self.use_operation(job_id, job_type_enum, **kwargs)
        elif job_type_enum == JobType.CONFIG_LOAD:
            coro = self.load_config(job_id, job_type_enum, **kwargs)
        elif job_type_enum == JobType.CONFIG_UPDATE:
            coro = self.update_config(job_id, job_type_enum, **kwargs)
        elif job_type_enum == JobType.CONFIG_SAVE:
            coro = self.save_config(job_id, job_type_enum, **kwargs)
        self.job_map[job_id] = (job_type_enum, coro)

        await self.job_queue.put(job_id)

        logging.info(f"Queued new {job_type_enum.value} job {job_id}")
        return job_id

    async def cancel_job(self, job_id: str, reason: str = None):
        if job_id not in self.job_map:
            raise NonexistantJobException(f"Job {job_id} does not exist or already finished")

        cancel_message = f"Setting job {job_id} to cancel"
        if reason:
            cancel_message += f" because {reason}"
            ...
            try:
                ...
                if self.job_current_id in self.job_skips:
                    ...
                    await self._handle_broadcast_error(
                        self.job_current_id, job_type, asyncio.CancelledError(reason)
                    )
    ...
    def get_loaded_operations(self):
        ...
        for key in op_d:
            if isinstance(op_d[key], Operation):
                ...
                op_d[key] = [x.op_id for x in op_d[key]]
            else:
                op_d[key] = "unknown"
    ...
    async def response_pipeline(self, job_id: str, job_type: JobType, include_audio: bool = True):

        # Adjust flags based on loaded ops
        if not self.op_manager.get_operation(OpRoles.TTS):
            include_audio = False
        ...
        if self.op_manager.get_operation(OpRoles.MCP):
            ...
            mcp_sys_prompt, mcp_user_prompt = (
                self.prompter.generate_mcp_system_context(),
                self.prompter.generate_mcp_user_context(),
            )
            tooling_response = ""
            async for chunk in self.op_manager.use_operation(
                OpRoles.MCP,
                {"instruction_prompt": mcp_sys_prompt, "messages": [RawMessage(mcp_user_prompt)]},
            ):
        ...
        # Appy t2t
        t2t_result = ""
        async for chunk_out in self.op_manager.use_operation(
            OpRoles.T2T, {"instruction_prompt": instruction_prompt, "messages": history}
        ):
            t2t_result += chunk_out["content"]

        # Broadcast raw results
        await self._handle_broadcast_event(
            job_id, job_type, {"instruction_prompt": instruction_prompt}
        )
        await self._handle_broadcast_event(
            job_id, job_type, {"history": [msg.to_dict() for msg in history]}
        )
        await self._handle_broadcast_event(job_id, job_type, {"raw_content": t2t_result})

        # Apply text filters
        async for text_chunk_out in self.op_manager.use_operation(
            OpRoles.FILTER_TEXT, {"content": t2t_result}
        ):
            ...
            if include_audio:
                # Apply tts
                async for audio_chunk_out in self.op_manager.use_operation(
                    OpRoles.TTS, text_chunk_out
                ):
                    # Apply tts filters
                    async for final_audio_chunk_out in self.op_manager.use_operation(
                        OpRoles.FILTER_AUDIO, audio_chunk_out
                    ):
                        # Broadcast results (only the audio data for now)
                        for ws_chunk in chunk_buffer(
                            base64.b64encode(final_audio_chunk_out["audio_bytes"]).decode("utf-8")
                        ):
                            await self._handle_broadcast_event(
                                job_id,
                                job_type,
    ...
    async def clear_context(self, job_id: str, job_type: JobType):
    ...
    async def configure_context(
        ...
        if history_length:
            payload |= {"history_length": history_length}
        if instruction_prompt_filename:
            payload |= {"instruction_prompt_filename": instruction_prompt_filename}
        if character_prompt_filename:
            payload |= {"character_prompt_filename": character_prompt_filename}
        if scene_prompt_filename:
            payload |= {"scene_prompt_filename": scene_prompt_filename}

        self.prompter.configure(payload)

        await self._handle_broadcast_success(job_id, job_type)

    async def append_request_context(self, job_id: str, job_type: JobType, content: str = None):
    ...
    async def append_conversation_context_text(
        ...
        await self._handle_broadcast_start(
            job_id, job_type, {"user": user, "timestamp": timestamp, "content": content}
        )
    ...
    async def append_conversation_context_audio(
        ...
        )  # Don't send full audio bytes over websocket, just flag as gotten
        audio_bytes: bytes = base64.b64decode(audio_bytes)
        prompt = (
            self.prompter.get_history_text() or f"You're name is {self.prompter.character_name}"
        )
        content = ""
        async for out_d in self.op_manager.use_operation(
            OpRoles.STT,
            {"prompt": prompt, "audio_bytes": audio_bytes, "sr": sr, "sw": sw, "ch": ch},
        ):
    ...
    async def register_custom_context(
        ...
        self.prompter.register_custom_context(
            context_id, context_name, context_description=context_description
        )
        await self._handle_broadcast_success(job_id, job_type)

    async def remove_custom_context(self, job_id: str, job_type: JobType, context_id: str = None):
    ...
    async def load_operations(
        self, job_id: str, job_type: JobType, ops: list[dict[str, str]] = None
    ):
        if ops is None:
            ops = []
        ...
        for op_d in ops:
            await self.op_manager.load_operation(
                OpRoles(op_d.get("role", None)), op_d.get("id", None), op_d.get("config", {})
            )
            await self._handle_broadcast_event(
                job_id,
                job_type,
    ...
    async def unload_operations(
        ...
        await self._handle_broadcast_start(job_id, job_type, {"ops": ops})
        for op_d in ops:
            await self.op_manager.close_operation(
                OpRoles(op_d.get("role", None)), op_d.get("id", None)
            )
            await self._handle_broadcast_event(
                job_id, job_type, {"role": op_d.get("role", None), "id": op_d.get("id", None)}
            )
        await self._handle_broadcast_success(job_id, job_type)
    ...
    async def configure_operations(  # TODO document and add endpoint
        ...
        await self._handle_broadcast_start(job_id, job_type, {"ops": ops})
        for op_d in ops:
            await self.op_manager.configure(
                OpRoles(op_d.get("role", None)), op_d, op_id=op_d.get("id", None)
            )
    ...
    async def use_operation(
        ...
        if "messages" in payload:
            ...
            for msg in payload["messages"]:
                ...
                if msg["type"] == "raw":
                    ...
                    msg_list.append(
                        RequestMessage(msg["message"], datetime.datetime.fromtimestamp(msg["time"]))
                    )
                elif msg["type"] == "chat":
                    msg_list.append(
                        ChatMessage(
                            msg["user"],
                            msg["message"],
                            datetime.datetime.fromtimestamp(msg["time"]),
                        )
                    )
                elif msg["type"] == "tool":
                    msg_list.append(
                    ...
                    msg_list.append(
                        CustomMessage(
                            msg["id"], msg["message"], datetime.datetime.fromtimestamp(msg["time"])
                        )
                    )
                else:
        ...
        try:
            ...
            async for chunk_out in op(payload):
                if "audio_bytes" in chunk_out:
                    chunk_out["audio_bytes"] = base64.b64encode(chunk_out["audio_bytes"]).decode(
                        "utf-8"
                    )
                await self._handle_broadcast_event(job_id, job_type, chunk_out)
            await op.close()
    ...
    async def _handle_broadcast_start(self, job_id: str, job_type: JobType, payload: dict):
        to_broadcast = {"job_id": job_id, "start": payload}
        logging.debug(f"Broadcasting start ({job_id}) {job_type.value} {str(to_broadcast):.500}")
    ...
    async def _handle_broadcast_event(self, job_id: str, job_type: JobType, payload: dict):
        to_broadcast = {"job_id": job_id, "finished": False, "result": payload}
        logging.debug(f"Broadcasting event ({job_id}) {job_type.value} {str(to_broadcast):.500}")
        await self.event_server.broadcast_event(job_type.value, to_broadcast)

    async def _handle_broadcast_success(self, job_id: str, job_type: JobType):
        to_broadcast = {"job_id": job_id, "finished": True, "success": True}
        logging.debug(f"Broadcasting success ({job_id}) {job_type.value} {str(to_broadcast)}")
        await self.event_server.broadcast_event(job_type.value, to_broadcast)

    async def _handle_broadcast_error(self, job_id: str, job_type: JobType, err: Exception):
        # TODO: extend with all errors
        error_type = "unknown"
        if isinstance(err, UnknownOpType):
            error_type = "operation_unknown_type"
        if isinstance(err, UnknownOpRole):
            error_type = "operation_unknown_role"
        elif isinstance(err, UnknownOpID):
            error_type = "operation_unknown_id"
        elif isinstance(err, DuplicateFilter):
            error_type = "operation_duplicate"
        elif isinstance(err, OperationUnloaded):
            error_type = "operation_unloaded"
        elif isinstance(err, StartActiveError):
            error_type = "operation_active"
        elif isinstance(err, CloseInactiveError):
            error_type = "operation_inactive"
        elif isinstance(err, UsedInactiveError):
            error_type = "operation_inactive"
        elif isinstance(err, UnknownField):
            error_type = "config_unknown_field"
        elif isinstance(err, UnknownFile):
            error_type = "config_unknown_file"
        elif isinstance(err, UnknownJobType):
            error_type = "job_unknown"
        ...
        to_broadcast = {
            ...
            "finished": True,
            "success": False,
            "result": {"type": error_type, "reason": str(err)},
        }
